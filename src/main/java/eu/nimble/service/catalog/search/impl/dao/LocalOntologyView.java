...
public class LocalOntologyView {
	...
	private String objectPropertySource = null;
	...
	/**
	 * Find a local ontology view for a concept. The concept could from any path
	 * start from the current local ontology view.
	 * 
	 * @param conceptURI
	 ...
	 * @return LocalOntologyView
	 */
	public LocalOntologyView findLocalOntologyViewForConceptFromRoot(String conceptURI, List<String> conceptURIPath,
			LocalOntologyView rootOntologyView) {
	...
	/**
	 * Delete all child invisible local ontology views from the the current
	 * local ontology view. When there is visible local ontology view within
	 * invisible local ontology view, only the visible ontology view will be
	 * kept.
	 * 
	 * e.g. Given Ontology View "HighChair(not hidden)-->Manufacture (hidden)
	 * --> Legislation (not hidden) Return: "HighChair(not hidden) -->
	 * Legislation (not hidden)
	 * 
	 * @return local ontology view for the displaying of the circle structure in
	 ...
	 * 
	 * Example 0. Given Ontology View "HighChair(not hidden) --> Legislation
	 * (not hidden) Return: "HighChair(not hidden) --> Legislation (not hidden)
	 * 
	 * Example 1. Given Ontology View "HighChair(not hidden)-->Manufacture
	 * (hidden) --> Legislation (not hidden) Return: "HighChair(not hidden) -->
	 * Manufacture/Legislation (not hidden) Rather than Return: "HighChair(not
	 * hidden) --> Legislation (not hidden) Because otherwise the user cannot
	 * know where the Legislations come from in the example 2.
	 * 
	 * Example 2. Given Ontology View
	 ...
	 * "HighChair(not hidden) --> Material/Legislation (not hidden) Rather than:
	 * "HighChair(not hidden) --> Legislation (not hidden) "HighChair(not
	 * hidden) --> Material/Legislation (not hidden)
	 * 
	 * Example 3. Given Ontology View
	 ...
	 * If the update is not expected, please make a deep copy of the current
	 * LocalOntologyView, and then use the deep copy to call the method.
	 * 
	 * @return local ontology view for the displaying of the circle structure in
	 *         the explorative search
	 */
			...
			for (String objProp : oldObjProps) {
				...
				if (tempVisible != null) {
					...
					if (visibleObjPropName.equals(objProp)) {
						...
						// Get new name for the visible LocalOntologyView with
						// some path information for the purpose of distinction
	...
	/**
	 * It is only used by the method
	 * "convertToVisibleLocalOntologyViewStructure", to get the name for visible
	 * LocalOntologyView that has some invisible parent node
	 * 
	 * e.g. In case of
	 * "HighChair(not hidden)-->Material (hidden) --> Legislation (not hidden)"
	 * The return name for "Legislation" is "Material/Legislation". The name
	 * include name of all parent nodes except the root node, because the root
	 * node shows always in the circle.
	 * 
	 * @param view
	 *            visible LocalOntologyView
	 * @return new name of the visible LocalOntologyView with some path
	 *         information for the purpose of distinction
	 */
	private static String getConceptNameWithPath(LocalOntologyView view) {
		...
		// the first element is always the root, and is not necessary to be
		// shown in the path
	...
	/**
	 * Find the single visible child local ontology view from the current hidden
	 * local ontology view. At the moment, under a invisible local ontology view
	 * e.g. Manufactuer, only one child local ontology view is visible e.g.
	 * Legislation.
	 * 
	...
	public void hiddenElementsOfParentView(LocalOntologyView parentLocalView) {
		...
		// todo....
		this.concept.setTranslatedURL(
				parentLocalView.getConcept().getTranslatedURL() + "/" + this.concept.getTranslatedURL());
	...
	public String toString() {
		return "LocalOntologyView [concept=" + concept + ", dataproperties=" + dataproperties + ", objectproperties="
				+ objectproperties + ", frozenConcept=" + frozenConcept + ", distanceToFrozenConcept="
				+ distanceToFrozenConcept + "]";
	}
	...
	public String getObjectPropertySource() {
		return objectPropertySource;
	}

	public void setObjectPropertySource(String objectPropertySource) {
		this.objectPropertySource = objectPropertySource;
	}

	public boolean isAReferenceToASourceObjectPropertyAvailable() {
		return (objectPropertySource != null) ? true: false;
	}
